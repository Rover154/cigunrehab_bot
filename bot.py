...
(
    ...
    ASK_FEEDBACK, BROADCAST_WAITING, MESSAGE_WAITING
) = range(11)
...
openai.base_url = "https://api.intelligence.io.solutions/api/v1"  # ← обновлено
...
async def ask_height_weight(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
...
async def ask_diagnoses_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
async def ask_diagnosis_timing(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
async def save_diagnosis_timing(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
async def ask_mobility(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text.strip()
...
async def ask_wellbeing(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Уведомление админа о новом клиенте
    if is_new_client:
        try:
...
async def generate_complex(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    profile = context.user_data.get("profile", {})

    diagnoses_text = [f"• {d['type']}: {d['timing']}" for d in profile.get("diagnoses_details", [])]
    ...
    try:
        # Лучше использовать асинхронный клиент, но для простоты оставляем синхронный
        # (можно потом заменить на aiohttp + openai.AsyncOpenAI)
...
Отвечайте кратко (до 250 слов), только на русском."""
        ...
        logger.error(f"Ошибка генерации комплекса: {e}")
        return ConversationHandler.END
...
async def handle_feedback_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "feedback_good":
        text = f"🌟 Отлично! Для персонализированной программы напишите {ADMIN_TELEGRAM}"
    elif query.data == "feedback_neutral":
        text = f"🧘 Главное — регулярность! Напишите {ADMIN_TELEGRAM} для подбора упражнений"
    elif query.data == "feedback_bad":
        text = f"😔 Проконсультируйтесь с врачом. Инструктор поможет адаптировать практики: {ADMIN_TELEGRAM}"
    else:
        text = f"💬 Напишите подробнее инструктору: {ADMIN_TELEGRAM}"
...
async def main():
    ...
    if not RENDER_EXTERNAL_HOSTNAME:
        logger.error("RENDER_EXTERNAL_HOSTNAME не найден! Невозможно установить webhook.")
        return
    ...
    logger.info("Webhook успешно запущен. Бот готов к приёму сообщений.")
    # Держим процесс живым
    await asyncio.Event().wait()


if __name__ == "__main__":
    print("🌿 Цигун-Реабилитация — запуск webhook-режима для Render.com")
    asyncio.run(main())